...
export async function novaCategoria(req: authorizationInfoRequest, res: Response) {
    ...
    try {
        const categoria = await categoriaService.criarCategoria(req.body.nome, req.body.usuarioId, res)
...
export async function obterCategoriaPorId(req: Request, res: Response) {
    ...
    try {
        const categoria = await categoriaService.obterCategoriaPorId(String(req.params.id), res)
        if (!categoria)
            return
...
export async function atualizarCategoria(req: Request, res: Response) {
    ...
    try {
        const categoria = await categoriaService.atualizarCategoria(String(req.params.id), req.body, res)
        if (!categoria)
            return
        res.status(200).json(categoria)
...
export async function deletarCategoria(req: authorizationInfoRequest, res: Response) {
    ...
    try {
        await categoriaService.deletarCategoria(String(req.params.id), res)
        res.status(204).send()
